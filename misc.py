...
class Misc(commands.Cog):
  ...
  async def reply_with_HUH(self, message: discord.Message):
    ...
    from_arcane = ['powder', 'jinx', 'vi', 'violet', 'jayce', 'caitlyn', 'ekko', 'silco', 'heimer', 'viktor']
    has_character = len([True for char in from_arcane if re.findall(rf'\b{char}\b', message.content.lower())]) > 0
    if has_character:
